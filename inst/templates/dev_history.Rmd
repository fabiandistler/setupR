---
title: "Development actions history"
output: html_document
editor_options: 
  chunk_output_type: console
---

All commands that you use to use when developing packages...

# First time just after creating the project

- Fill the following chunk to create the DESCRIPTION of your package

```{r description}
# Describe your package
fusen::fill_description(
  pkg = here::here(),
  fields = list(
    Title = "Build A Package From Rmarkdown File",
    Description = "Use Rmarkdown First method to build your package. Start your package with documentation. Everything can be set from a Rmarkdown file in your project.",
    `Authors@R` = c(
      person("John", "Doe", email = "john@email.me", role = c("aut", "cre"), comment = c(ORCID = "0000-0000-0000-0000"))
    )
  )
)
# Define License with use_*_license()
usethis::use_mit_license("John Doe")
```

# Make sure everything is set up correctly for development

```{r, eval=FALSE}
usethis::git_sitrep()

usethis::proj_sitrep()

devtools::dev_sitrep()

usethis::use_testthat()
```

# All-in-one function to deploy publicly on GitHub

Either use this GitHub all-in-one function or choose the steps one-by-one in the following sections.
See `vignette("share-on-a-github-website", package = "fusen")` for more information.

```{r, eval=FALSE}
# _All-in-one share of your package and its documentation on GitHub
fusen::init_share_on_github()
```


# Start using git

```{r, eval=FALSE}
usethis::use_git()
# Deal with classical files to ignore
usethis::git_vaccinate()
# Use main for primary branch
usethis::git_default_branch_rename()
```

# Set up `{targets}` pipeline for analysis.
If you have packaged your data preparation code, you might want to create
a separate branch/repo for you pipeline and load you package in it.

# Create branch set up for analsis package and targets.
```{r, eval=FALSE}
ger::
```

# Targets
```{r, eval=FALSE}
targets::use_targets() # Can use targets::use_targets_rmd() instead.
# Interactive app to set up pipeline.
targetsketch::targetsketch()
targets::tar_visnetwork()
targets::tar_manifest()
targets::tar_sitrep()
# Can use different data store e.g. for sensitive data.
# targets::tar_config_set(store = "path/to/store")
targets::tar_make()
# Get pipeline deps for renv.
targets::tar_renv()
# Run pipeline on github.
targets::tar_github_actions()
```


# Set extra sources of documentation

```{r, eval=FALSE}
# Install a first time
remotes::install_local()
# README
usethis::use_readme_rmd()
# NEWS can be used as changelog.
usethis::use_news_md()
```

**From now, you will need to "inflate" your package at least once to be able to use the following commands. Let's go to your flat template, and come back here later if/when needed.**


# Package development tools
## Use once

```{r, eval=FALSE}
# Pipe
usethis::use_pipe()

# package-level documentation
usethis::use_package_doc()

# GitHub
# Add your credentials for GitHub
usethis::create_github_token()
gitcreds::gitcreds_set()

# _Or one step at a time
# Send your project to a new GitHub project (included in `init_share_on_github()`)
usethis::use_github()

# Set Continuous Integration
# _GitHub (included in `init_share_on_github()`)
usethis::use_github_action("pkgdown")
usethis::use_github_action("test-coverage")
usethis::use_github_action("pr-commands")
usethis::use_github_action("style")
# _GitLab
gitlabr::use_gitlab_ci(type = "check-coverage-pkgdown")
```

## Use everytime needed

```{r, eval=FALSE}
# Simulate package installation
pkgload::load_all()

# Generate documentation and deal with dependencies
attachment::att_amend_desc(
  # pkg_ignore = c(), # Add packages to ignore.
  extra.suggests = c("devtools", "renv", "pkgdown", "fusen"), # Possible defaults.
  update.config = TRUE
)

# Renv
renv::init()
attachment::create_renv_for_dev()
attachment::create_renv_for_prod()

# Check the package
devtools::check()

# Add a new flat template
fusen::add_flat_template("add")

# Inflate all flat files at once
fusen::inflate_all()

# Package structure (add this in a dev/Readme.md file)
fusen::draw_package_structure()
```

# Share the package

```{r, eval=FALSE}
# set and try pkgdown documentation website locally
usethis::use_pkgdown()
pkgdown::build_site()

# build the tar.gz with vignettes to share with others
devtools::build(vignettes = TRUE)

# Share your package and its documentation on GitHub
# usethis::create_github_token()
# gitcreds::gitcreds_set()
fusen::init_share_on_github()
```
